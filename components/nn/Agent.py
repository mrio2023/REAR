...
def make_linear_block(in_dim, out_dim, activation=Swish, norm_type=None):
    """Create a linear block with optional normalization and activation."""
...
class Agent(nn.Module):
    ...
    def forward(self, x_seeds, x_nodes, indptr):
        ...
        Args:
            x_seeds: seed node embeddings [total_nodes, embed_dim]
            x_nodes: trajectory node embeddings [total_nodes, embed_dim]
            indptr: array of (start, end, candidate_end) indices per batch element

        Returns:
            List of logits tensors, one per batch element.
        """
